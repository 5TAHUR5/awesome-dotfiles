...
updateWalls = function()
	awful.spawn.easy_async(
		{ "sh", "-c", "ls -1 -F --indicator-style=none " .. paths.to_dir_lite_walls .. " | tr '\n' '|'" },
		function(stdout)
			walls = {}
			for wall in stdout:gmatch("[^|]+") do
...
updateLuts = function()
	awful.spawn.easy_async({ "sh", "-c", "ls " .. paths.to_dir_luts .. " | grep .png | tr '\n' '|'" }, function(stdout)
		luts = {}
		for lut in stdout:gmatch("[^|]+") do
			table.insert(luts, lut)
		end
		table.remove(luts)
...
updateLiteWalls = function()
	...
	awful.spawn.easy_async({
		...
		"-c",
		"rm -rf "
			.. paths.to_dir_lite_walls
			.. "* && magick mogrify -resize 160x100 -path "
			.. paths.to_dir_lite_walls
			.. " "
			.. paths.to_dir_walls
			.. "*",
...
local lut_how_sys_btn = create_walls_btn("SYNC AWESOME THEME")
...
local set_selectable_lut = function(lut_name)
	if lut_name == "" or lut_name == "lut_how_system" then
		...
		if selectable_lut == "" then
			...
			lut_how_sys_btn:set_bg(beautiful.background_alt)
		else
			off_lutgen_btn:set_bg(beautiful.background_alt)
...
local create_lut_box = function(lut_name, luts_path)
	...
	ret_lut_box:buttons({ awful.button({}, 1, function()
		set_selectable_lut(lut_name)
	end) })
	return ret_lut_box
end
...
local create_wall_box = function(wall_name, walls_path)
	if wall_name then
		...
		ret_wall_box:buttons({ awful.button({}, 1, function()
			set_selectable_wall(wall_name)
		end) })
		return ret_wall_box
	else
		return wibox.widget({
...
awesome.connect_signal("theme_changer::update_walls", function(walls, walls_path, current_wall)
	...
	if #walls ~= 0 then
		...
		for i = 1, #walls, 3 do
			walls_wrapper:insert(
				#walls_wrapper.children + 1,
				create_walls_row(walls[i], walls[i + 1], walls[i + 2], walls_path)
			)
		end
